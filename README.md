...
Coordination service for AI coding agents. Agents declare a plan before writing code; the service compares it against every other open plan and the team's shared memory and answers `proceed`, `proceed_with_context`, or `wait`. Conflicts are detected on **concepts and stance**, never on file paths.

Spec: [consensus-backend-build-handoff-v9.md](consensus-backend-build-handoff-v9.md). Website copy (getting started, how it works, features, pricing): [docs/](docs/README.md).
