...
export default function LandingApp() {
  ...
  const teamClick = (id: string)   => { setSelTeamId(id); navigate(`/app/dashboard?team=${encodeURIComponent(id)}`); };
          ...
          <div className="admin-teams-list">
            {teams.map(t => (
              <div key={t.id} className="admin-team-row">
                ...
                <button className="secondary-button sm-btn" onClick={() => navigate(`/app/dashboard?team=${encodeURIComponent(t.id)}`)}>Open Dashboard <ArrowRight size={13} /></button>
              </div>
            ))}
          </div>
        )}
